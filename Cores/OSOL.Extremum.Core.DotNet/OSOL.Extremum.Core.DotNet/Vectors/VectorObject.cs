...
namespace OSOL.Extremum.Core.DotNet.Vectors
{
    public abstract class VectorObject<TBase>
    {
        ...
        public IEnumerable<Tuple<String, TBase>> ElementWiseOp(VectorObject<TBase> that, Func<TBase, TBase, TBase> op)
        {
        ...
        public IEnumerable<Tuple<String, TBase>> ElementWiseOpImputeMissingKeys(VectorObject<TBase> that, Func<TBase, TBase, TBase> op, TBase defaultValue)
        {
            List<string> mergedKeys = new List<string>(this.Keys);
            foreach (string k in that.Keys)
                if(!mergedKeys.Contains(k)) mergedKeys.Add(k);
            return mergedKeys.Select(k => Tuple.Create(k, op(this[k, defaultValue], that[k, defaultValue])));
        }
        ...
        public Dictionary<string, TBase> Neg() => this.Multiply(coefficient: -1.0);
