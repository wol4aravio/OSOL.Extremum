...
class RemoteFunctionsSuite extends FunSuite {
  ...
  test("Dummy Real f") {
    ...
    val f = new RealRemoteFunction(json = s"${sys.env("OSOL_EXTREMUM_TASKS_LOC")}/Dummy/Dummy_3.json", port = 11110, field = "f")
    f.initialize()
  ...
  test("Dummy Interval f") {
    ...
    val f = new IntervalRemoteFunction(json = s"${sys.env("OSOL_EXTREMUM_TASKS_LOC")}/Dummy/Dummy_3.json", port = 11120, field = "f")
    f.initialize()
