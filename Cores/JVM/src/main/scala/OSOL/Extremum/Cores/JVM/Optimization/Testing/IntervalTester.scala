...
object IntervalTester {
  ...
  val f1: IntervalRemoteFunction = new IntervalRemoteFunction(json = s"${TASKS_LOC}/Dummy/Dummy_1.json", port = 5021, field = "f")
  val a1: Area = vars_1.map(k => (k, (-10.0, 10.0))).toMap
  val s1: Map[String, Double] = vars_1.map(k => (k, 0.0)).toMap

  val f2: IntervalRemoteFunction = new IntervalRemoteFunction(json = s"${TASKS_LOC}/Dummy/Dummy_2.json", port = 5022, field = "f")
  val a2: Area = vars_2.map(k => (k, (-10.0, 10.0))).toMap
  val s2: Map[String, Double] = vars_2.map(k => (k, 0.0)).toMap

  val f3: IntervalRemoteFunction = new IntervalRemoteFunction(json = s"${TASKS_LOC}/Dummy/Dummy_3.json", port = 5023, field = "f")
