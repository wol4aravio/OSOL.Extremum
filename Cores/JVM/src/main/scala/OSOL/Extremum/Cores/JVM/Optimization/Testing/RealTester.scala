...
object RealTester {
  ...
  val f1: RealRemoteFunction = new RealRemoteFunction(json = s"${TASKS_LOC}/Dummy/Dummy_1.json", port = 11111, field = "f")
  val a1: Area = vars_1.map(k => (k, (-10.0, 10.0))).toMap
  val s1: Map[String, Double] = vars_1.map(k => (k, 0.0)).toMap

  val f2: RealRemoteFunction = new RealRemoteFunction(json = s"${TASKS_LOC}/Dummy/Dummy_2.json", port = 11112, field = "f")
  val a2: Area = vars_2.map(k => (k, (-10.0, 10.0))).toMap
  val s2: Map[String, Double] = vars_2.map(k => (k, 0.0)).toMap

  val f3: RealRemoteFunction = new RealRemoteFunction(json = s"${TASKS_LOC}/Dummy/Dummy_3.json", port = 11113, field = "f")
