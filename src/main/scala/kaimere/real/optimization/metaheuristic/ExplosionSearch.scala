...
case class ExplosionSearch(numberOfBombs: Int, powerRatio: Double, numberOfDims: Int)
  ...
  case class Bomb(location: RealVector, fitness: Double) {
    ...
    def explode(power: Map[String, Double], f: Function, area: OptimizationAlgorithm.Area): (Bomb, Bomb) = {
      ...
      val explodingKeys = GoRN.getFromSeries(power.keys.toSeq, numberOfDims, withReturn = false)

      val Seq(leftRightKey) = GoRN.getFromSeries(explodingKeys, 1, withReturn = false)
      val bothSidesKeys = explodingKeys.toSet - leftRightKey
      val zeroKeys = power.keySet -- explodingKeys

      val delta = (zeroKeys.map(key => (key, (0.0, 0.0))) ++ bothSidesKeys.map(key => (key, (-power(key), power(key))))).toMap[String, (Double, Double)]
      val deltaLeft = delta + (leftRightKey -> (-power(leftRightKey), 0.0))
      val deltaRight = delta + (leftRightKey -> (0.0, power(leftRightKey)))
