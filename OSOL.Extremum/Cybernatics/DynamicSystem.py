...
class DynamicSystem:
    ...
    def __init__(self,
        ...
        self.state_vars = state_vars + ['I_integral_{}'.format(i + 1)
        ...
        self.terminal_criterion = lambdify(self.sym_vars[:(1 + len(self.state_vars))],
        ...
        for i in range(len(self.phase_constraints)):
            self.f['phase_{}'.format(i + 1)] = lambdify(self.sym_vars,
                                                        parse_expr('phase({0}, {1}, {2})'.format(self.phase_constraints[i]['equation'],
                                                                                                 self.phase_constraints[i]['penalty'],
                                                                                                 float(self.phase_constraints[i]['norm'][1:]))), [np, {'phase': phase}])
            self.initial_conditions['phase_{}'.format(i + 1)] = 0.0

        self.aux = aux
        counter = 0
        for v in self.etc_vars:
            self.aux[v] = lambdify(self.sym_vars[:(len(self.sym_vars) - (len(self.etc_vars) - counter))],
                                   parse_expr(self.aux[v]), np)
            counter += 1
        ...
        for i in range(len(self.terminal_constraints)):
            self.terminal_constraints[i]['equation'] = lambdify(self.sym_vars[:(len(self.sym_vars) - len(self.etc_vars) - len(self.control_vars))],
                                                                parse_expr(self.terminal_constraints[i]['equation']), np)
